...
namespace SkillBank.Site.DataSource.Data
{
    ...
    public class OrderRepository : DbContext, IOrderRepository
    {
        ...
        {
            return Order_LoadByStudent_p(studentId);
         }


        public List<OrderItem> GetOrderListByTeacher(int teacherId)
        {
            return Order_LoadByTeacher_p(teacherId);
        }
        ...
        /// <returns></returns>
        private ObjectResult<Order> Order_Load(Byte loadType, int paraId)
        {
            var loadTypeParameter = new ObjectParameter("LoadType", loadType);
            var idParameter = new ObjectParameter("ParaId", paraId);

            ObjectResult<Order> result = ((IObjectContextAdapter)this).ObjectContext.ExecuteFunction<Order>("Order_Load_p", MergeOption.NoTracking, loadTypeParameter, idParameter);
            return result;
        }
        ...
        /// <param name="paraId"></param>
        /// <returns></returns>
        private List<OrderItem> Order_LoadByStudent_p(int paraId)
        {
            var idParameter = new ObjectParameter("ParaId", paraId);

            ObjectResult<OrderItem> result = ((IObjectContextAdapter)this).ObjectContext.ExecuteFunction<OrderItem>("Order_LoadByStudent_p", MergeOption.NoTracking, idParameter);
            return OrderMapper.Map(result);
        }
        ...
        /// <param name="paraId"></param>
        /// <returns></returns>
        private List<OrderItem> Order_LoadByTeacher_p(int paraId)
        {
            var idParameter = new ObjectParameter("ParaId", paraId);

            ObjectResult<OrderItem> result = ((IObjectContextAdapter)this).ObjectContext.ExecuteFunction<OrderItem>("Order_LoadByTeacher_p", MergeOption.NoTracking, idParameter);
            return OrderMapper.Map(result);
