...
namespace dotNetDR_OAuth2.APIs.Providers.WeChat
{
    public class WeChatApi : IApi
    {
        public const string API_BASE_URL = "https://api.weixin.qq.com/";
        private static AppConfig _APP_CONFIG = AppConfigs.GetWeChat();
        ...
        private dynamic Call(string url, string httpMethod, string accessToken, string openid, IDictionary<string, string> formData, IDictionary<string, BinaryData> binaryData, bool returnJson = false)
        {
            ...
            var queryStringOAuthLan = "&lang=zh_CN";
            ...
            ClientRequest cr = new ClientRequest(url + queryStringAccessToken + queryStringOpenid + queryStringOAuthLan);
        ...
        public bool WasError<T>(dynamic obj, out T error) where T : class, IError
        {
            ...
            {
                if (typeof(T) != typeof(WeChatError))
                {
                    throw new Exception("泛型T的类型必须为 dotNetDR_OAuth2.APIs.Providers.WeChat.WeChatError");
                }
